...
def drawBenchmarks(len_quota,benchmark_framelevel,benchmark_videolevel,ben_name):
    ...
    pl.savefig("../data/dataset3_result/d3_fl_"+ben_name+"_double.pdf")
   ...
   # pl.vlines(5, 0.85, 1, color='black', linestyles='dashed')
    ...
    plt.xlabel('cN')
    plt.ylabel('Detection Performance')
    pl.savefig("../data/dataset3_result/d3_vl_"+ben_name+"_double.pdf")
...
ahash_framelevel = np.load('../data/dataset3/roc_framelevel_data_ahash.npy', allow_pickle=True)
ahash_videolevel = np.load('../data/dataset3/roc_videolevel_data_ahash.npy', allow_pickle=True)
...
for i in range(len_quota):
    ...
    pl.savefig("../data/dataset3_result/d3_roc_fl_"+str(i)+".pdf")
    ...
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    pl.savefig("../data/dataset3_result/d3_roc_vl_"+str(i)+".pdf")
...
pl.plot(sample_nums[1:],benchmarks_framelevel_insertion[1:], label='CCSHG - Object Insertion')
#pl.hlines((precision_ahash_framelevel+recall_ahash_framelevel+F1_ahash_framelevel+iou_ahash_framelevel)/4,0,10,color='blue',linestyles='dashed',label="ahash")
pl.hlines((benchmarks_framelevel[0]+benchmarks_framelevel_insertion[0])/2,1,len_quota-1,color='red',linestyles='dashed',label="baseline")
#pl.vlines(5,0.6,0.82,color='black',linestyles='dashed')
plt.legend()
x_major_locator=MultipleLocator(1)
#把x轴的刻度间隔设置为1，并存在变量里
...
plt.xlabel('cN')
plt.ylabel('Detection Performance')
pl.savefig("../data/dataset3_result/d3_fl_double.pdf")
...
pl.plot(sample_nums[1:],benchmarks_videolevel_insertion[1:], label='CCSHG - Object Insertion')
#pl.hlines((precision_ahash_videolevel+recall_ahash_videolevel+F1_ahash_videolevel+iou_ahash_videolevel)/4,0,10,color='blue',linestyles='dashed',label="ahash")
pl.hlines((benchmarks_videolevel[0]+benchmarks_videolevel_insertion[0])/2,1,len_quota-1,color='red',linestyles='dashed',label="baseline")
pl.vlines(5,0.85, 1,color='black',linestyles='dashed')
plt.legend()
x_major_locator=MultipleLocator(1)
#把x轴的刻度间隔设置为1，并存在变量里
...
plt.xlabel('cN')
plt.ylabel('Detection Performance')
pl.savefig("../data/dataset3_result/d3_vl_double.pdf")

drawBenchmarks(len_quota,precision_framelevel,precision_videolevel,"precision")
drawBenchmarks(len_quota,recall_framelevel,recall_videolevel,"recall")
drawBenchmarks(len_quota,F1_framelevel,F1_videolevel,"F1")
drawBenchmarks(len_quota,iou_framelevel,iou_videolevel,"iou")