...
def preprocessing(images, size=(256, 256), r_len=256):
    img_array = []
    v_len = len(images)

    for i in range(v_len):
        frame = images[i]
        lu_lib = np.array(Image.fromarray(frame).convert('L'))
       # b, g, r = cv2.split(frame)
      #  lu_paper=np.array((65.481 * r) + (128.553 * g) + (24.966 * b)+16)
     #   img_array.append(lu_paper)
       #  print("lu_lib",lu_lib)
       #  print("lu_paper",lu_paper)
       # # print("h",np.array(lu.resize(size)))
       #  print("lu_eq",(0.299  * r) + (0.587 * g) + ( 0.114 * b)+16)
       #  print("lu_eq1", (0.299 * r) + (0.587 * g) + (0.114 * b) )
       #  print("lu_eq2", (0.257  * r) + (0.504 * g) + (0.098 * b)+16)
    ...
    for i in range(len(data_path)):
        ...
        for file in os.listdir(path):
            ...
            if '.mp4' in file or '.avi' in file:
                ...
                db_path = path + '/' + file_name + '_compared_LRH.npy'
