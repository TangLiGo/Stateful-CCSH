...
def get5Metrics(labels, scores,level):
    ...
    thresholds = np.arange(0, np.max(np.array(scores)), 0.01)
    #print(thresholds)
    tps = []
    fps = []
    tns = []
   ...
   # forged_info = [[100, 133], [111, 187], [99, 172], [95, 181], [64, 72], [1, 32], [103, 143], [120, 138], [77, 131],
    ...
    for i in range(len(nums_forgery)):  # 第几组
        for j in range(start,len(nums_forgery[i])):  # 第几个
          ...
          #   plt.show()
            processed_hd = get_frame_info(nums_forgery[i][j][0], segment_size, video_len[j])
            frame_scores.extend(processed_hd)
            video_scores.append(nums_forgery[i][j][1])
         #   video_scores.append(np.average(np.array(nums_forgery[i][j][0])))
         #   print(forged_info[j],len(nums_forgery[i][j]))
            frame_labels.extend([0 for j in range(forged_info[j][0])]+[1 for j in range(forged_info[j][0],forged_info[j][1])]+[0 for j in range(forged_info[j][1],len(processed_hd))])
            video_labels.append(1)

    for i in range(len(nums_compression)):
        for j in range(len(nums_compression[i])):
            processed_hd = get_frame_info(nums_compression[i][j][0], segment_size, video_len[j])
            frame_scores.extend(processed_hd)
           # video_scores.append(np.average(np.array(nums_compression[i][j][0])))
            video_scores.append(nums_compression[i][j][1])
            # plt.figure()
            # plt.plot(nums_compression[i][j])
            # plt.show()
            frame_labels.extend([0 for k in range(len(processed_hd))])
...
def hash_compare(video_path1,video_path2):
    H_set1=np.array(np.load(video_path1,allow_pickle=True))
    H_set2 = np.array(np.load(video_path2, allow_pickle=True))
...
def traverseVideo(dataset=4):
    ...
    for i in range(len(data_path)):
        ...
        for file in os.listdir(path):
            ...
            if ('Real' in file or 'original' in file) and "_compared_LRH.npy" in file:
                original_videos[i].append(video_path)
            elif ('Forged' in file or 'forged' in file)and "_compared_LRH.npy" in file:
...
np.save('data/dataset1_LRH_hash_distances.npy',[hds_forgery,hds_compression])
...
np.save('data/dataset4_LRH_hash_distances.npy',[hds_forgery,hds_compression])
