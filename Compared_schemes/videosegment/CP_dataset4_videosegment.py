...
def compare_hd(insertion_num,seq):
    ...
    for i in range(len(data_path)):
        ...
        for file in os.listdir(path):
            ...
            if 'Real' in file and '.mp4' in file:
                original_videos[i].append(video_path)
            elif 'Forged' in file and '.mp4' in file:
        ...
        for j in range(len(forged_videos)):
            ...
            for k in range(len(original_videos[i])):
                if k in [0,1,7,8,9]:
                    ...
                    sub_hds_forgery.append([])
            hds_forgery.append(sub_hds_forgery)
    hds_compression=[]
    return hds_forgery, hds_compression


insertion_nums=range(1,6)#[5078.234375, 5073.171875, 5081.953125, 5100.796875, 5290.890625]
repeatTime=1
time_costs=[]
for insertion_num in insertion_nums:
    ...
    if insertion_num<10:
        np.save('data/hds_ahash_sample_advanced_0' + str(insertion_num) + '_segment_m1_choose.npy', hds_forgery_arr)
        np.save('data/hds_ahash_sample_advanced_0' + str(insertion_num) + '_segment_m1_choose_compression.npy',
                hds_compression_arr)
    else:
        np.save('data/hds_ahash_sample_advanced_' + str(insertion_num) + '_segment_m1_choose.npy', hds_forgery_arr)
        np.save('data/hds_ahash_sample_advanced_' + str(insertion_num) + '_segment_m1_choose_compression.npy',
                hds_compression_arr)
