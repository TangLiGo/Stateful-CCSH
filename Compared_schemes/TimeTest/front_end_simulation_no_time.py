...
def baseline1(imgs):
    ...
    np.save('data/baseline1_320.npy', [path_1920,frame_hash_values, sampling_lists])
...
def PVH(imgs):
  ...
  #  print("3d", H_3D[0][1][1])
   ...
   # print("2d",len(H_2D),len(H_2D[0]))
    np.save('data/PVH_320.npy', [path_1920, [H_2D, V_2D], []])
...
def baseline2(imgs):
    ...
    for f in imgs:
        ...
        cur_frame_hash = imagehash.average_hash(Image.fromarray(np.array(blocks)))
        frame_hash_values.append((cur_frame_hash))
    np.save('data/baseline2_320.npy', [path_1920,frame_hash_values,sampling_lists])
def CCSH(imgs):
    ...
    for f in imgs:
        ...
        cur_frame_hash = imagehash.average_hash(Image.fromarray(np.array(blocks)))
        frame_hash_values.append((cur_frame_hash))
    np.save('data/CCSH_320.npy', [path_1920,frame_hash_values, sampling_lists])
def ahash(imgs):
    ...
    np.save('data/ahash_320.npy', [path_1920, frame_hash_values])
