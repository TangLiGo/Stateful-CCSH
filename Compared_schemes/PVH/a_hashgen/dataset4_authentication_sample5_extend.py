...
def get_frame_info(hd,segment_size,video_len):
    ...
    output.extend([hd[-1] for j in range(video_len-len(output))])
...
def traverseVideo():
    ...
    for i in range(len(data_path)):
        ...
        for file in os.listdir(path):
            ...
            if 'Real' in file and "compared_perceptualhash" in file:
                original_videos[i].append(video_path)
            elif 'Forged' in file and "compared_perceptualhash" in file:
...
np.save('data/dataset4_authen_sample5.npy',[hds_forgery,hds_compression])
hds_forgery,hds_compression=np.load('data/dataset4_authen_sample5.npy',allow_pickle=True)
