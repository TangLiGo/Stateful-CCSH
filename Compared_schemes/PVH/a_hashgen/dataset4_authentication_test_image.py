...
def hash_compare(video_path1,video_path2):
    ...
    for q in range(Q):
        for i in range(M):
            for j in range(N):
                Dq[q]+=Arr1[q][i][j]
               # if q==163 and Arr1[q][i][j]==1:
                 #   print('posA,',i,j)#27-45,24-33
    im=Image.fromarray(np.array(Arr1[-1]))
    print(Dq[-1])
    plt.imshow(im)  # 显示新图片
    imageio.imwrite('new_img.jpg', im)  # 保存图片到本地
    video_path='C:/Users/tangli/Desktop/research2_dataset/dataset4_revised/Lossless/Real_004.mp4'
    ...
    while True:
        ...
        if not success:
            break
        if frame_id==163*5:
            imageio.imwrite('original_img.jpg',frame)
...
data_path0 = 'C:/Users/tangli/Desktop/research2_dataset/dataset4_revised/Lossless/Real_004_compared_all_frame_perceptualhash.npy'
data_path1 = 'C:/Users/tangli/Desktop/research2_dataset/dataset4_revised/Lossy2/Forged_004_lossy2_compared_all_frame_perceptualhash.npy'

re = (hash_compare(data_path0, data_path1))