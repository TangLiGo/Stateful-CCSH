...
def data_processing(nums_forgery,nums_compression):
    ...
    for i in range(len(nums_forgery)):  # 第几组
        for j in range(1,len(nums_forgery[i])):  # 第几个
            print("nums_forgery[i][j]",len(nums_forgery[i][j]))
...
def traverseVideo():
    ...
    for i in range(len(data_path)):
        ...
        for file in os.listdir(path):
            ...
            if 'original' in file and "compared_all_frame_perceptualhash" in file:
...
np.save('data/dataset1_authen_all_frame.npy',[hds_forgery,hds_compression])
hds_forgery,hds_compression=np.load('data/dataset1_authen_all_frame.npy',allow_pickle=True)
