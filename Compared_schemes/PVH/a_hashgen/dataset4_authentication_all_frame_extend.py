...
def get5Metrics(labels, scores,level):
    ...
    thresholds = np.arange(0, np.max(np.array(scores)), 1)
    tps = []
    fps = []
    tns = []
    fns = []
  ...
  #  print("tpr_compared=",tpr)
  #  print("fpr_compared=", fpr)
...
def traverseVideo():
    ...
    for i in range(len(data_path)):
        ...
        for file in os.listdir(path):
            ...
            if 'Real' in file and "compared_all_frame_perceptualhash" in file:
                original_videos[i].append(video_path)
            elif 'Forged' in file and "compared_all_frame_perceptualhash" in file:
...
#np.save('data/dataset4_authen_all_frame.npy',[hds_forgery,hds_compression])
#hds_forgery,hds_compression=np.load('data/dataset4_authen_all_frame.npy',allow_pickle=True)
data_processing(hds_forgery,hds_compression)
##get frame metrics by segment number