...
for i in range(len(sample_nums)):
    ...
    if sample_nums[i]<10:
        np.save('../data/roc_videolevel_00' + str(sample_nums[i]) + '.npy',
                np.array([fpr_videolevel_temp, tpr_videolevel_temp, precision_videolevel_temp,
                          recall_videolevel_temp, F1_videolevel_temp, iou_videolevel_temp],
                         dtype=object))
        np.save('../data/roc_framelevel__00' + str(sample_nums[i]) + '.npy',
                np.array([fpr_framelevel_temp, tpr_framelevel_temp, precision_framelevel_temp,
                          recall_framelevel_temp, F1_framelevel_temp, iou_framelevel_temp],
                         dtype=object))
    elif sample_nums[i]<100:
        np.save('../data/roc_videolevel_0' + str(sample_nums[i]) + '.npy',
                np.array([fpr_videolevel_temp, tpr_videolevel_temp, precision_videolevel_temp,
                          recall_videolevel_temp, F1_videolevel_temp, iou_videolevel_temp],
                         dtype=object))
        np.save('../data/roc_framelevel_0' + str(sample_nums[i]) + '.npy',
                np.array([fpr_framelevel_temp, tpr_framelevel_temp, precision_framelevel_temp,
                          recall_framelevel_temp, F1_framelevel_temp, iou_framelevel_temp],
                         dtype=object))
    else:
        np.save('../data/roc_videolevel_' + str(sample_nums[i]) + '.npy',
                np.array([fpr_videolevel_temp, tpr_videolevel_temp, precision_videolevel_temp,
                          recall_videolevel_temp, F1_videolevel_temp, iou_videolevel_temp],
                         dtype=object))
        np.save('../data/roc_framelevel_' + str(sample_nums[i]) + '.npy',
                np.array([fpr_framelevel_temp, tpr_framelevel_temp, precision_framelevel_temp,
                          recall_framelevel_temp, F1_framelevel_temp, iou_framelevel_temp],
                         dtype=object))
