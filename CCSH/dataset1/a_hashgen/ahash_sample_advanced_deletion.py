        ...
        if img_i != 0:
           ...
           # print("last",last_sampling_list_row)
            ...
            sampling_list.extend(np.random.randint(low=0, high=row_block_range * col_block_range,
                                               size=(sample_num - sample_deletion,)))
           ...
           # for i in range(sample_deletion):
               # index_temp = last_sampling_list[cor_blocks.index(sorted_blocks[i])]
               # cor_blocks[cor_blocks.index(sorted_blocks[i])]=2
...
for insertion_num in insertion_nums:
    ...
    if insertion_num<10:
        np.save('../data_deletion/hds_ahash_sample_advanced_0' + str(insertion_num) + '_deletion.npy', hds_forgery_arr)
        np.save('../data_deletion/hds_ahash_sample_advanced_0' + str(insertion_num) + '_deletion_compression.npy',
                hds_compression_arr)
    else:
        np.save('../data_deletion/hds_ahash_sample_advanced_' + str(insertion_num) + '_deletion.npy', hds_forgery_arr)
        np.save('../data_deletion/hds_ahash_sample_advanced_' + str(insertion_num) + '_deletion_compression.npy',
                hds_compression_arr)
