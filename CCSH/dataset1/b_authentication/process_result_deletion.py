...
def drawBenchmarks(len_quota,benchmark_framelevel,benchmark_videolevel,ben_name):
    ...
    # pl.plot(sample_nums[1:],F1_framelevel_deletion[1:], label='CCSH - Object deletion')
    ...
    # pl.plot(sample_nums[1:],F1_videolevel_deletion[1:], label='CCSH - Object deletion')
...
ahash_framelevel = np.load('../data_deletion/roc_framelevel_data_ahash.npy', allow_pickle=True)
ahash_videolevel = np.load('../data_deletion/roc_videolevel_data_ahash.npy', allow_pickle=True)
...
for i in range(len_quota):
    ...
    pl.plot( fpr_framelevel_deletion[0], tpr_framelevel_deletion[0], label='baseline',color='darkorange')
   # pl.plot(fpr_framelevel_temp, tpr_framelevel_temp, label='CCSH')
    pl.plot(fpr_framelevel_deletion_temp, tpr_framelevel_deletion_temp, label='CCSH',color='seagreen')
    ...
    pl.savefig("../result/d1_roc_fl_"+str(i)+"_deletion.pdf")

    pl.figure()
    pl.plot(fpr_ahash_videolevel,tpr_ahash_videolevel,label='ahash',color='steelblue')
    pl.plot(fpr_videolevel_deletion[0], tpr_videolevel_deletion[0], label='baseline',color='darkorange')
  #  pl.plot(fpr_videolevel_temp, tpr_videolevel_temp, label='CCSH')
    pl.plot(fpr_videolevel_deletion_temp, tpr_videolevel_deletion_temp, label='CCSH',color='seagreen')
    plt.legend(fontsize=12)
    plt.ylim((0, 1))
    plt.xlim((0, 1))
    plt.xlabel('False Positive Rate',fontsize=12)
    ...
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    pl.savefig("../result/d1_roc_vl_"+str(i)+"_deletion.pdf")




pl.figure()
pl.plot(sample_nums[:6],F1_framelevel_deletion[:6], label='CCSH',color='seagreen')

#pl.hlines((precision_ahash_framelevel+recall_ahash_framelevel+F1_ahash_framelevel+iou_ahash_framelevel)/4,0,10,color='blue',linestyles='dashed',label="ahash")
pl.hlines(F1_framelevel_deletion[0],0,5,color='darkorange',linestyles='dashed',label="baseline")
#pl.vlines(5,0.6,0.82,color='black',linestyles='dashed')
plt.legend(fontsize=12)
x_major_locator=MultipleLocator(1)
#把x轴的刻度间隔设置为1，并存在变量里
...
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
pl.savefig("../result/d1_fl_single_deletion.pdf")


pl.figure()
pl.plot(sample_nums[:6],F1_videolevel_deletion[:6], label='CCSH',color='seagreen')

pl.hlines(F1_videolevel_deletion[0],0,5,color='darkorange',linestyles='dashed',label="baseline")
...
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
pl.savefig("../result/d1_vl_single_deletion.pdf")